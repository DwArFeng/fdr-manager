<template>
  <div>
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>过滤数据查询</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 卡片工作区 -->
    <el-card class="box-card">
      ...
      <div style="margin-top: 15px;">
        <el-row :gutter="20">
          <el-col :span="7">
            <el-input
              placeholder="请输入数据点ID或过滤器ID，仅支持精确查询"
        ...
        <el-col>
          <div class="pagination-container">
            <el-pagination
              ...
              :page-size="pageSize"
              :total="parseInt(filteredValue.count)"
    ...
    <el-dialog
      ...
      <el-form
        ...
        status-icon
        :model="anchorFilteredValue"
        ref="detailForm">
        <el-form-item label="ID" prop="key">
          <el-input
            v-model="anchorFilteredValue.key"
            :disabled="true"
          ></el-input>
        </el-form-item>
        <el-form-item label="数据点ID" prop="pointKey">
          <el-input
            v-model="anchorFilteredValue.point_key"
            :disabled="true"
          ></el-input>
        </el-form-item>
        <el-form-item label="过滤器ID" prop="filterKey">
          <el-input
            v-model="anchorFilteredValue.filter_key"
            :disabled="true"
          ></el-input>
        </el-form-item>
        ...
        <el-form-item label="数据值" prop="value">
          <el-input
            v-model="anchorFilteredValue.value"
            type="textarea"
            autosize
            :readonly="true"
          ></el-input>
        ...
        <el-form-item label="信息" prop="message">
          <el-input
            v-model="anchorFilteredValue.message"
            type="textarea"
            autosize
            :readonly="true"
          ></el-input>
...
export default {
  name: 'Permission',
  data() {
    const validateFilteredValueNotExists = (rule, value, callback) => {
      if (value === '') {
        callback();
      } else {
        exists(value)
    ...
    return {
      ...
      createPermissionRules: {
        key: [
          {
            validator: validateFilteredValueNotExists,
            trigger: 'blur',
          },
        ],
        name: [
  ...
  methods: {
    ...
    lookupChildForFilter() {
      childForFilter(this.id2Search, this.currentPage - 1, this.pageSize)
        .then((res) => {
          if (res.meta.code !== 0) {
            this.$message({
              showClose: true,
          ...
          // 当查询的页数大于总页数，自动查询最后一页。
          if (res.data.current_page >= res.data.total_pages && res.data.total_pages > 0) {
            return childForFilter(this.id2Search, res.data.total_pages - 1, this.pageSize);
          }
          this.filteredValue = res.data;
          this.currentPage = res.data.current_page + 1;
          return null;
    ...
    lookupChildForFilterBetween() {
      childForFilterBetween(this.id2Search, this.dateRange[0], this.dateRange[1],
        this.currentPage - 1, this.pageSize)
        .then((res) => {
          if (res.meta.code !== 0) {
            this.$message({
          ...
          // 当查询的页数大于总页数，自动查询最后一页。
          if (res.data.current_page >= res.data.total_pages && res.data.total_pages > 0) {
            return childForFilterBetween(this.id2Search, this.dateRange[0], this.dateRange[1],
              res.data.total_pages - 1, this.pageSize);
          }
          this.filteredValue = res.data;
          this.currentPage = res.data.current_page + 1;
    ...
    handleSearch() {
      if (this.idType === 'point') {
        ...
        this.lookupChildForFilter();
      } else if (this.id2Search !== '' && this.dateRange != null) {
        this.lookupChildForFilterBetween();
      }
    },
    handleShowDetailDialog(row) {
      ...
      this.anchorFilteredValue.key = row.key.long_id;
      this.anchorFilteredValue.point_key = row.point_key.long_id;
      this.anchorFilteredValue.filter_key = row.filter_key.long_id;
      this.anchorFilteredValue.happened_date = row.happened_date;
      this.anchorFilteredValue.value = row.value;
      this.anchorFilteredValue.message = row.message;
    ...
    refreshTimer() {
      if (this.useTimer && this.detailVisible) {
        this.timer = setInterval(() => {
          inspect(this.anchorFilteredValue.key)
            .then((res) => {
              if (res.meta.code !== 0) {
                return null;
              }
              this.anchorFilteredValue.key = res.data.key.long_id;
              this.anchorFilteredValue.happened_date = res.data.happened_date;
              this.anchorFilteredValue.value = res.data.value;
              return null;
            }).catch(() => null);
        }, 1000);
  ...
  computed: {
    formattedDate() {
      const timestamp = this.anchorFilteredValue.happened_date;
      // 时间戳为10位需*1000，时间戳为13位的话不需乘1000 var date = new Date(timestamp*1000);
      const date = new Date(timestamp);
      const Y = `${date.getFullYear()}-`;
      const M = `${date.getMonth() + 1 < 10 ? `0${date.getMonth() + 1}` : date.getMonth() + 1}-`;
